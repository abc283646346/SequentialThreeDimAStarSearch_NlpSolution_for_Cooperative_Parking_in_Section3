function fitness = PlanXYTAStarPathForRankingqualityEvaluation(iv)
global Nfe
...
costmap_{final_path_time}(final_path_x,final_path_y) = 1;
path_length = 0;
while (cur_best_parent_index(1) ~= -999)
    ...
    costmap_{final_path_time(1)}(final_path_x(1), final_path_y(1)) = 1;
    path_length = path_length + norm([final_path_x(1)-final_path_x(2), final_path_y(1)-final_path_y(2)]);
end
fitness = 20 * (abs(final_path_x(end) - end_index(1)) + abs(final_path_y(end) - end_index(2))) + path_length;
end
